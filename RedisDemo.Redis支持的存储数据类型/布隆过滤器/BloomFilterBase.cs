...
namespace RedisDemo
{
    public abstract class BloomFilterBase<T>
    {
        ...
        protected HashFunction getHashSecondary;
        protected int sizeOfBloomFilter;
        ...
        public BloomFilterBase(int capacity) : this(capacity, null) { }
        ...
        public BloomFilterBase(int capacity, float errorRate) : this(capacity, errorRate, null) { }
        ...
        public BloomFilterBase(int capacity, HashFunction hashFunction) : this(capacity, bestErrorRate(capacity), hashFunction) { }
        ...
        /// <param name="errorRate">The accepable false-positive rate (e.g., 0.01F = 1%)</param>
        /// <param name="hashFunction">The function to hash the input values. Do not use GetHashCode(). If it is null, and T is string or int a hash function will be provided for you.</param>
        public BloomFilterBase(int capacity, float errorRate, HashFunction hashFunction) : this(capacity, errorRate, hashFunction, bestM(capacity, errorRate), bestK(capacity, errorRate)) { }
        ...
        public BloomFilterBase(int capacity, float errorRate, HashFunction hashFunction, int m, int k)
        {
        ...
        //    get
        //    {
        //        return (double)trueBits() / sizeOfBloomFilter;
        //    }
        //}
        ...
        /// </summary>
        protected int computeHash(int primaryHash, int secondaryHash, int i)
        {
            int resultingHash = (primaryHash + (i * secondaryHash)) % sizeOfBloomFilter;
            return Math.Abs((int)resultingHash);
        }

        protected static int bestK(int capacity, float errorRate)
        {
            return (int)Math.Round(Math.Log(2.0) * bestM(capacity, errorRate) / capacity);
        }

        protected static int bestM(int capacity, float errorRate)
        {
            return (int)Math.Ceiling(capacity * Math.Log(errorRate, (1.0 / Math.Pow(2, Math.Log(2.0)))));
        }

        protected static float bestErrorRate(int capacity)
        {
        ...
        protected static int hashInt32(T input)
        {
        ...
        /// <returns>The hashed result.</returns>
        protected static int hashString(T input)
        {
